...
void test12()
{
    const substitution s("flee at once. we are discovered!", "ZEBRASCDFGHIJKLMNOPQTUVWXY");
    const std::string encoded_message = s.encode();

    std::cout << encoded_message << std::endl;
    std::cout << s.decode(encoded_message) << std::endl;
}

void test13()
{
    const playfair s("WHEATSON", "IDIOCY OFTEN LOOKS LIKE INTELLIGENCE");
    const std::string encoded_message = s.encode();
...
void test14()
{
    const hill s(3, "ACT", "GYBNQKURP");

    const std::string encoded = s.encode();
    std::cout << "Encoded: " << encoded << std::endl;

    const std::string decoded = s.decode(encoded);
