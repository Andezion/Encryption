...
private:
    ...
    static U16 UnalignedLoad16(const void* p)
    {
        return *static_cast<const U16*>(p);
    }
    static U32 UnalignedLoad32(const void* p)
    {
        return *static_cast<const U32*>(p);
    }

    static void UnalignedStore16(void* p, const U16 x)
    {
        *static_cast<U16*>(p) = x;
    }
    static void UnalignedCopy32(void* d, const void* s)
    {
        *static_cast<U32*>(d) = UnalignedLoad32(s);
    }
...
public:
    ...
    {
        const int max_chain = level >= 8 ? WINDOW_SIZE : 1 << level;
        U8* op = out;
        ...
        while (p < in_len)
        {
            ...
            if (const size_t max_match = in_len - p; max_match >= MIN_MATCH)
            {
                ...
                while (s > limit)
                {
                    ...
                    {
                        int len = MIN_MATCH;
                        while (len < static_cast<int>(max_match) && in[s + len] == in[p + len])
                        {
                            ++len;
                        }
    ...
    static size_t decompressBuffer(const U8* in, const size_t in_len, U8* out, const size_t out_len)
    {
        ...
        while (ip < ip_end)
        {
            ...
            if (tag >= 32)
            {
                ...
                if (run == 7)
                {
                    run += DecodeMod(const_cast<U8*&>(ip));
                }
            ...
            if (len == 15 + MIN_MATCH)
            {
                len += DecodeMod(const_cast<U8*&>(ip));
            }
    ...
    static void decompressFile(const std::string& inFile, const std::string& outFile)
    {
        ...
        if (magic != MAGIC)
        {
            throw std::runtime_error("Not ULZ format");
        }
