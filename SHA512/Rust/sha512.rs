...
#[rustfmt::skip]
pub const ROUND_CONSTANTS_: [u64; 80] = [
...
pub fn convert_bits_to_u64_target<F: RichField + Extendable<D>, const D: usize>(
    ...
    let mut msg_u64: Vec<U64Target> = Vec::new();
    let len = msg.len();
    let mut i = 0usize;
    while i + 64 <= len {
      
        let low_slice = &msg[i..i + 32];
        let high_slice = &msg[i + 32..i + 64];
      
        let u32_limb_0 = U32Target(builder.le_sum(low_slice.iter().rev()));
        let u32_limb_1 = U32Target(builder.le_sum(high_slice.iter().rev()));
        
        let target = U64Target {
            limbs: [u32_limb_1, u32_limb_0],
        };
        msg_u64.push(target);
        i += 64;
    }
...
pub fn add_u64targets<F: RichField + Extendable<D>, const D: usize>(
    ...
    let (high_addition_lo_carry_lo, _high_addition_lo_carry_hi) =
        builder.add_u32(high_addition_lo, low_addition_hi);
    U64Target {
        limbs: [low_addition_lo, high_addition_lo_carry_lo],
    }
}
...
pub fn sha512_circuit_with_preprocessed_input<F: RichField + Extendable<D>, const D: usize>(
    ...
    for i in (0..input_u64.len()).step_by(16) {
        ...
        for j in 16..80 {
            ...
            let s0 = U64Target {
                limbs: builder.unsafe_xor_many_u64(&[s0_0, s0_1, s0_2.limbs]),
            };
            ...
            let s1 = U64Target {
                limbs: builder.unsafe_xor_many_u64(&[s1_0, s1_1, s1_2.limbs]),
            };
            let sum = add_u64targets(
                builder,
        ...
        for j in 0..80 {
            ...
            let sum1 = U64Target {
                limbs: builder.unsafe_xor_many_u64(&[sum1_0, sum1_1, sum1_2]),
            };
            ...
            let ch = U64Target {
                limbs: builder.unsafe_xor_many_u64(&[ch_0, ch_1]),
            };

            let temp1 = add_u64targets(
                builder,
                &[h.clone(), sum1, ch, round_constants_u64_target[j].clone(), w[j].clone()],
            );
            ...
            let sum0 = U64Target {
                limbs: builder.unsafe_xor_many_u64(&[sum0_0, sum0_1, sum0_2]),
            };
            ...
            g = f;
            f = e;
            e = add_u64targets(builder, &[d.clone(), temp1.clone()]);
            d = c;
            c = b;
            b = a;
...
pub fn make_sha512_circuit<F: RichField + Extendable<D>, const D: usize>(
    ...
    let mut preprocessed_input_len = 0usize;
...
fn prove_sha512(msg: &[u8]) -> Result<()> {
    ...
    let targets = make_sha512_circuit(&mut builder, len_bits as u128);
    ...
    println!(
        "Time taken to build the circuit the proof - {:?}",
        circuit_build_time.elapsed()
    );
