...
static ecc_uint ecc_vli_num_digits(const uint64_t *v)
{
    for (int i = NUM_ECC_DIGITS - 1; i >= 0; --i)
    {
        if (v[i])
        {
            return static_cast<ecc_uint>(i + 1);
        }
    }
    return 0;
}
...
{
    if (constexpr uint32_t max_bits = NUM_ECC_DIGITS * 64U; bit >= max_bits)
    {
        return 0;
    }
...
{
    uint64_t minus_a[NUM_ECC_DIGITS];
    ecc_vli_clear(minus_a);
    minus_a[0] = 3;
...
static int ecdsa_sign(const uint8_t p_private[ECC_BYTES], const uint8_t p_hash[ECC_BYTES], uint8_t p_signature[ECC_BYTES * 2])
{
    ...
    do
    {
        ...
        if (ecc_vli_cmp(curve_n, p.x) <= 0)
        {
            ecc_vli_sub(p.x, p.x, curve_n);
        }
    }
...
static int ecdsa_verify(const uint8_t p_public[ECC_BYTES + 1], const uint8_t p_hash[ECC_BYTES], const uint8_t p_signature[ECC_BYTES * 2])
{
    ...
    if (ecc_vli_is_zero(r) || ecc_vli_is_zero(s))
    {
        return 0;
    }
    if (ecc_vli_cmp(curve_n, r) <= 0 || ecc_vli_cmp(curve_n, s) <= 0)
    {
        return 0;
    }
