...
class BWT {
public:
    static std::string encode(const std::string &t) {
        int suffsize = t.size() + 3;
        int *txt = new int[suffsize];
        for (size_t i = 0; i < suffsize; i++) {
            if (i < t.size())
                txt[i] = reinterpret_cast<const unsigned char &>(t[i]);
            else
                txt[i] = 0;
        }
    ...
    static std::string decode(std::string t) {
        ...
        std::string s_index = t.substr(0, indexTerminatingChar);
        int index = std::stoi(s_index, nullptr, 10);
...
private:
    ...
    static void suffixArray(int *T, int *SA, int n, int K) {
...
#define GetI() (SA12[t] < n0 ? SA12[t] * 3 + 1 : (SA12[t] - n0) * 3 + 2)
    ...
    static std::string invert(std::string &BWTArray, long long index) {
        int BWTLength = BWTArray.size();
        std::vector<unsigned char> BWTSortedArray(BWTLength);
        for (size_t i = 0; i < BWTLength; i++)
            BWTSortedArray[i] = (reinterpret_cast<unsigned char &>(BWTArray[i]));

        std::vector<int> shift;
        unsigned int x = index;
        std::sort(BWTSortedArray.begin(), BWTSortedArray.end());

        std::vector<std::vector<int>> arr(256);
