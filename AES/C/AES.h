#pragma once

#include <stdint.h>
#include <stdlib.h>

#ifdef __cplusplus
extern "C"
{
#endif
    uint8_t *aes_init(size_t key_size);
    void aes_key_expansion(const uint8_t *key, uint8_t *w);
    void aes_cipher(const uint8_t *in, uint8_t *out, const uint8_t *w);
    void aes_inv_cipher(const uint8_t *in, uint8_t *out, const uint8_t *w);

#ifdef __cplusplus
}
#endif


static int Nb = 4;
static int Nk;
static int Nr;

static const uint8_t s_box[256] = {
    
};

static const uint8_t inv_s_box[256] = {
    
};

static uint8_t gmult(uint8_t a, uint8_t b)
{
    uint8_t p = 0;
    for (int i = 0; i < 8; i++)
    {
        if (b & 1)
        {
            p ^= a;
        }

        const uint8_t hi = a & 0x80;
        a <<= 1;

        if (hi)
        {
            a ^= 0x1b;
        }

        b >>= 1;
    }
    return p;
}

static void coef_add(const uint8_t a[4], const uint8_t b[4], uint8_t d[4])
{
    d[0] = a[0] ^ b[0];
    d[1] = a[1] ^ b[1];
    d[2] = a[2] ^ b[2];
    d[3] = a[3] ^ b[3];
}

static void coef_mult(const uint8_t a[4], const uint8_t col[4], uint8_t res[4])
{
    res[0] = gmult(a[0], col[0]) ^ gmult(a[3], col[1]) ^ gmult(a[2], col[2]) ^ gmult(a[1], col[3]);
    res[1] = gmult(a[1], col[0]) ^ gmult(a[0], col[1]) ^ gmult(a[3], col[2]) ^ gmult(a[2], col[3]);
    res[2] = gmult(a[2], col[0]) ^ gmult(a[1], col[1]) ^ gmult(a[0], col[2]) ^ gmult(a[3], col[3]);
    res[3] = gmult(a[3], col[0]) ^ gmult(a[2], col[1]) ^ gmult(a[1], col[2]) ^ gmult(a[0], col[3]);
}

static uint8_t *Rcon(uint8_t i)
{
    static uint8_t R[4] = {0x00, 0x00, 0x00, 0x00};

    if (i == 1)
    {
        R[0] = 0x01;
    }
    else if (i > 1)
    {
        R[0] = 0x02;
        i--;

        while (i > 1)
        {
            R[0] = gmult(R[0], 0x02);
            i--;
        }
    }
    return R;
}

static void add_round_key(uint8_t *state, const uint8_t *w, const uint8_t r)
{
    for (int c = 0; c < Nb; c++)
    {
        state[Nb * 0 + c] ^= w[4 * Nb * r + 4 * c + 0];
        state[Nb * 1 + c] ^= w[4 * Nb * r + 4 * c + 1];
        state[Nb * 2 + c] ^= w[4 * Nb * r + 4 * c + 2];
        state[Nb * 3 + c] ^= w[4 * Nb * r + 4 * c + 3];
    }
}

static void sub_bytes(uint8_t *state)
{
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < Nb; j++)
        {
            state[Nb * i + j] = s_box[state[Nb * i + j]];
        }
    }
}

static void inv_sub_bytes(uint8_t *state)
{
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < Nb; j++)
        {
            state[Nb * i + j] = inv_s_box[state[Nb * i + j]];
        }
    }
}

static void shift_rows(uint8_t *state)
{
    for (int i = 1; i < 4; i++)
    {
        for (int s = 0; s < i; s++)
        {
            const uint8_t tmp = state[Nb * i + 0];
            for (int k = 1; k < Nb; k++)
            {
                state[Nb * i + k - 1] = state[Nb * i + k];
            }
            state[Nb * i + Nb - 1] = tmp;
        }
    }
}

static void inv_shift_rows(uint8_t *state)
{
    for (int i = 1; i < 4; i++)
    {
        for (int s = 0; s < i; s++)
        {
            const uint8_t tmp = state[Nb * i + Nb - 1];
            for (int k = Nb - 1; k > 0; k--)
            {
                state[Nb * i + k] = state[Nb * i + k - 1];
            }
            state[Nb * i + 0] = tmp;
        }
    }
}

static void mix_columns(uint8_t *state)
{
    static constexpr uint8_t a[4] = {0x02, 0x01, 0x01, 0x03};
    uint8_t col[4];

    for (int j = 0; j < Nb; j++)
    {
        uint8_t res[4];
        for (int i = 0; i < 4; i++)
        {
            col[i] = state[Nb * i + j];
        }
        coef_mult(a, col, res);

        for (int i = 0; i < 4; i++)
        {
            state[Nb * i + j] = res[i];
        }
    }
}

static void inv_mix_columns(uint8_t *state)
{
    static constexpr uint8_t a[4] = {0x0e, 0x09, 0x0d, 0x0b};
    uint8_t col[4];

    for (int j = 0; j < Nb; j++)
    {
        uint8_t res[4];
        for (int i = 0; i < 4; i++)
        {
            col[i] = state[Nb * i + j];
        }

        coef_mult(a, col, res);
        for (int i = 0; i < 4; i++)
        {
            state[Nb * i + j] = res[i];
        }
    }
}

static void sub_word(uint8_t w[4])
{
    w[0] = s_box[w[0]];
    w[1] = s_box[w[1]];
    w[2] = s_box[w[2]];
    w[3] = s_box[w[3]];
}

static void rot_word(uint8_t w[4])
{
    const uint8_t tmp = w[0];
    w[0] = w[1];
    w[1] = w[2];
    w[2] = w[3];
    w[3] = tmp;
}

inline uint8_t *aes_init(const size_t key_size)
{
    switch (key_size)
    {
        case 16: Nk = 4; Nr = 10; break;
        case 24: Nk = 6; Nr = 12; break;
        case 32: Nk = 8; Nr = 14; break;
        default: return nullptr;
    }

    return static_cast<uint8_t *>(malloc(Nb * (Nr + 1) * 4 * sizeof(uint8_t)));
}

inline void aes_key_expansion(const uint8_t *key, uint8_t *w)
{
    uint8_t tmp[4];
    uint8_t i;
    const uint8_t len = Nb * (Nr + 1);

    for (i = 0; i < Nk; i++) {
        w[4 * i + 0] = key[4 * i + 0];
        w[4 * i + 1] = key[4 * i + 1];
        w[4 * i + 2] = key[4 * i + 2];
        w[4 * i + 3] = key[4 * i + 3];
    }

    for (i = Nk; i < len; i++)
    {
        tmp[0] = w[4 * (i - 1) + 0];
        tmp[1] = w[4 * (i - 1) + 1];
        tmp[2] = w[4 * (i - 1) + 2];
        tmp[3] = w[4 * (i - 1) + 3];

        if (i % Nk == 0)
        {
            rot_word(tmp);
            sub_word(tmp);

            coef_add(tmp, Rcon(i / Nk), tmp);
        }
        else if (Nk > 6 && i % Nk == 4)
        {
            sub_word(tmp);
        }

        w[4 * i + 0] = w[4 * (i - Nk) + 0] ^ tmp[0];
        w[4 * i + 1] = w[4 * (i - Nk) + 1] ^ tmp[1];
        w[4 * i + 2] = w[4 * (i - Nk) + 2] ^ tmp[2];
        w[4 * i + 3] = w[4 * (i - Nk) + 3] ^ tmp[3];
    }
}

inline void aes_cipher(const uint8_t *in, uint8_t *out, const uint8_t *w)
{
    uint8_t state[4 * Nb];
    uint8_t i, j;

    for (i = 0; i < 4; i++)
    {
        for (j = 0; j < Nb; j++)
        {
            state[Nb * i + j] = in[i + 4 * j];
        }
    }

    add_round_key(state, w, 0);

    for (uint8_t r = 1; r < Nr; r++)
    {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, w, r);
    }

    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, w, Nr);

    for (i = 0; i < 4; i++)
    {
        for (j = 0; j < Nb; j++)
        {
            out[i + 4 * j] = state[Nb * i + j];
        }
    }
}

inline void aes_inv_cipher(const uint8_t *in, uint8_t *out, const uint8_t *w)
{
    uint8_t state[4 * Nb];
    uint8_t i, j;

    for (i = 0; i < 4; i++)
    {
        for (j = 0; j < Nb; j++)
        {
            state[Nb * i + j] = in[i + 4 * j];
        }
    }

    add_round_key(state, w, Nr);

    for (uint8_t r = Nr - 1; r >= 1; r--)
    {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, w, r);
        inv_mix_columns(state);
    }

    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, w, 0);

    for (i = 0; i < 4; i++)
    {
        for (j = 0; j < Nb; j++)
        {
            out[i + 4 * j] = state[Nb * i + j];
        }
    }
}
